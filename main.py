...
from analysis import BehavioralAnalysisEngine
...
/_/  /_/  /___/  /_/ |_|  /_/  |_|\____/  /_____/   
    ...
    def watcher_callback(event):
        ...
        try:
            session_id = event.get("session", event.get("session_id", "unknown"))
            behavioral_engine.add_event(session_id, event)
        except Exception as e:
            print(f"[Main] Behavioral analysis error: {e}")

    log_watcher_thread = threading.Thread(target=log_watcher.start, args=(watcher_callback,), daemon=True)
...
def nexus_portal():
    try:
        ...
        return HTMLResponse("<h2>Portal UI not found.</h2>")
...
def dashboard_ui():
    try:
        ...
        return HTMLResponse("<h2>Dashboard UI not found.</h2>")
...
def session_ui():
    try:
        with open(os.path.join(STATIC_DIR, "session.html"), "r", encoding="utf-8") as f:
            return HTMLResponse(f.read())
    except FileNotFoundError:
        return HTMLResponse("<h2>Session UI not found.</h2>")
