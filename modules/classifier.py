...
def _load_model():
    ...
    if os.path.exists(MODEL_PATH):
        ...
        print(f"[Classifier] No trained model found at {MODEL_PATH}. Run: python modules/train_model.py")
    return _pipeline
...
def _llm_classify(session_data: dict) -> dict:
    client = _get_client()
    if not client:
...
def classify(session_data: dict) -> dict:
    ...
    print(f"[Classifier] ML confidence low ({result['confidence'] if result else 'N/A'}), consulting LLM...")
    ai_result = _llm_classify(session_data)
    if result:
        ai_result["ml_type"]       = result["type"]
        ai_result["ml_confidence"] = result["confidence"]
    return ai_result if ai_result else (result if result else _heuristic_classify(session_data))
