...
def _get_client():
    ...
    if _client is None and config.GROQ_API_KEY:
        try:
            ...
            print(f"[LiveSession] Failed to create Groq client: {e}")
    return _client
...
def _ai_comment(cmd: str, session_context: list) -> str:
    ...
    try:
        ...
        print(f"[LiveSession] AI comment fallback: {e}")
        return _heuristic_comment(cmd)
...
def command_executed(session_id: str, src_ip: str, cmd: str):
    ...
    heuristic_comment = _heuristic_comment(cmd)

    # Insert entry immediately with heuristic comment
    entry = {
        "cmd":        cmd,
        "ts":         time.time(),
        "ai_comment": heuristic_comment,
        "risk":       risk,
    }
    with _lock:
    ...
    # Async: try to get AI comment and update
    def _process():
        try:
            context = active_sessions.get(session_id, {}).get("commands", [])
            ai_comment = _ai_comment(cmd, context)
            if ai_comment and ai_comment != heuristic_comment:
                with _lock:
                    if session_id in active_sessions:
                        for entry in active_sessions[session_id]["commands"]:
                            if entry["cmd"] == cmd and entry["ai_comment"] == heuristic_comment:
            ...
            print(f"[LiveSession] AI update error: {e}")
